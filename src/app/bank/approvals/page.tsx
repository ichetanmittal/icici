    ...
    if (user) {
      ...
      try {
        ...
        if (data?.role === UserRole.DBS_BANK_MAKER) {
          const pendingResponse = await fetch(`/api/ptt-requests?status=pending&importerBank=DBS Bank`);
          const pendingData = await pendingResponse.json();
          if (pendingData.requests) {
            setPendingRequests(pendingData.requests);
          }

          const makerApprovedResponse = await fetch(`/api/ptt-requests?status=maker_approved&importerBank=DBS Bank`);
