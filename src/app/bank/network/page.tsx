...
export default function NetworkManagementPage() {
  ...
  const handleSaveLimit = async (memberId: string, memberType: string) => {
    try {
      const response = await fetch('/api/admin/update-credit-limit', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
    ...
    <div>
      ...
      <div className="mb-8 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600">Manage importers and funder banks in your network</p>
        </div>
        <button
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div ref={modalRef} className="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900">Invite Importer</h2>
              <button
