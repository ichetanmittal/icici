...
export async function POST(request: NextRequest) {
  try {
    ...
    if (memberType === 'Importer') {
      // Try updating user_profiles first (for active importers)
    ...
    } else if (memberType === 'Funder Bank') {
      // For funder banks, update treasury_balance in user_profiles
      const { error } = await supabase
        .from('user_profiles')
        .update({ treasury_balance: creditLimit })
        .eq('id', memberId);
    ...
    console.error('Error in update-credit-limit API:', error);
    return NextResponse.json(
      { error: 'Failed to update credit limit' },
      { status: 500 }
    );
